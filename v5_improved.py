"""
V5: Zero-padded FFT (441->2048) with physics-informed peak selection.
Solves FFT bin discretization — correctly resolves 35Hz from 30Hz.
Peak validation: a prominence-based comparison confirms results in 87% of
conditions and final sigma differs by <0.02 N/m (see Supplementary Material).

Usage:
  1. Edit EXPERIMENTS list below with your experiment name, cx, cy.
  2. Edit container_dir name if different from '圆形容器_10mm'.
  3. Run: python v5_improved.py
  4. Results saved to <实验图片>/<exp>/temporal_circular_results_v5.json
  5. Diagnostic images saved to 实验结果汇总/<freq>Hz/
"""
...
# ============================================================
SCALE = 30.0 / 235        # mm/px (30mm container radius / 235px wall radius)
ZP_SIZE = 2048             # zero-padding FFT size
RHO_LM, RHO_E, G = 6280.0, 1010.0, 9.81
H_LM, H_E = 0.01, 0.00106  # layer thicknesses (m)

# Experiment calibration — add one entry per experiment
#   name: folder name under 实验图片/
#   cx, cy: container center (px) — use container_detector.py to find
#   container_dir: subfolder name for this container type
EXPERIMENTS = [
...
def analyze_fft_zp(hp_patch, sq, scale, freq_drive):
    """Zero-padded FFT with physics-informed peak scoring."""
    ...
    for idx in local_max:
        ...
        candidates.append({
            ...
            'power': float(power_val), 'sigma': sigma,
        })
    ...
    def score(c):
        ...
        if 0.4 < sig < 1.3:
            ...
            s = s * 0.2 - 5
        lam_exp = 21.0 - freq_drive * 0.17
        lam_dev = abs(c['lam_true'] - lam_exp) / max(lam_exp, 1)
        if lam_dev < 0.15: s += 3
        elif lam_dev < 0.35: s += 0
        else: s -= 3
        return s
...
def process_exp(exp_cfg):
    ...
    print(f"\n{'='*85}")
    print(f"V5 ZP-FFT: {exp_cfg['name']} | {len(conditions)} cond | ZP={ZP_SIZE}px")
    print(f"{'='*85}")
    ...
    for i, cond in enumerate(conditions):
        ...
        if 'error' in r:
            ...
            # Save diagnostic images for first good result per frequency
        ...
        # Clean up image arrays from result dict
    ...
    if saved_freqs:
        print(f"  Diagnostics saved for {len(saved_freqs)} frequencies: {sorted(saved_freqs)}")

    jp = os.path.join(base, 'temporal_circular_results_v5.json')
